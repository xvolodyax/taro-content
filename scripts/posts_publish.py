...
def image_url(package: Path, slot: str) -> str:
    ...
    if url.startswith("http"):
        return url
    return "AgACAgQAAyEGAASN-fG2AAIK3GqbezzTmtgvEerE-fQjBoaOiTlzAAJ3EGsbL3m9UHr4gud0sCCiAQADAgADeAADPQQ" if slot == "alena" else ""
...
def build_destinations(slot: str) -> list[Dest]:
    ...
    if slot == "alena":
        dests = [Dest("telegram-alena", "telegram", tg_alias, tools["telegram_photo"], alena)]
        dests.append(Dest("max", "max", "", "MAX_SEND", skip=True, reason="нет MAX_BOT_TOKEN"))
        return dests
    raise ValueError(f"unknown slot {slot}")


def apply_optional_max(dests: list[Dest], slot: str = "") -> None:
    token = (os.environ.get("MAX_BOT_TOKEN") or "").strip()
    default_chat = "-74272905910873" if slot == "alena" else "AaBmlTdadaI"
    chat = (os.environ.get("MAX_CHAT_ID") or default_chat).strip()
    for dest in dests:
        if dest.name != "max":
            continue
        if not token:
...
def plan_package(package: Path, now: datetime | None = None) -> Plan:
    ...
    if current < when:
        ...
        apply_optional_max(plan.destinations, slot)
        return plan

    dests = build_destinations(slot)
    apply_optional_max(dests, slot)
    ...
    if need_image and not url:
        ...
        reason = "нет картинки (file_id / cover-url.txt / POST_IMAGE_URL)"
        if local:
        ...
        try:
            ...
            if isinstance(resp_data, dict):
                ...
                if msg_id:
                    ...
                    if dest.chat_id and str(dest.chat_id).startswith("@"):
                        row["link"] = f"https://t.me/{str(dest.chat_id).lstrip('@')}/{msg_id}"
                    elif dest.name == "telegram-alena":
                        row["link"] = f"https://t.me/AlenaSafonova_queen/{msg_id}"
            ledger_add(plan.date, plan.slot, dest.name, {"tool": dest.tool, "alias": dest.alias})
