...
def plan_package(package: Path, now: datetime | None = None) -> Plan:
    ...
    if current < when and not getattr(now, "_force", False):
        # Token-burn fix: never keep the cloud agent alive until 12:12 / 15:15 / 21:21.
        # PASS → READY_TO_SEND → EXIT. Air at slot = Hall / short wake.
        plan.status = "READY_TO_SEND"
        plan.reason = (
            "слот МСК не наступил — READY_TO_SEND, агент выходит. "
            "Эфир = Холл / короткий air wake, не живой WAIT."
        )
...
def wait_for_slot(plan: Plan, now: datetime | None = None) -> Plan:
    """Disabled: living WAIT/sleep burned Cursor balance (bc-a70b9897).

    Never sleep / poll / loop until 12:12 / 15:15 / 21:21.
    PASS → READY_TO_SEND → EXIT. Slot air is Hall / a short wake.
    """
    if plan.status in {"WAIT", "READY_TO_SEND"}:
        plan.status = "READY_TO_SEND"
        plan.reason = (
            "слот МСК не наступил — READY_TO_SEND, агент выходит. "
            "Эфир = Холл / короткий air wake, не живой WAIT."
...
def main() -> int:
    ...
    parser.add_argument(
        "--wait",
        action="store_true",
        help="DEPRECATED/disabled: не ждать слот. Пишет READY_TO_SEND и выходит.",
    )
    ...
    # --wait must not sleep or re-enter a Read-loop. Convert and exit.
    if plan.status in {"WAIT", "READY_TO_SEND"}:
    ...
    if plan.status in {"SKIP", "WAIT", "READY_TO_SEND"}:
        report = {
            "package": str(package),
            "date": plan.date,
            "slot": plan.slot,
