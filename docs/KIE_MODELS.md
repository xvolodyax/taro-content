...
## Разрешение и кадр (HARD 09.09)

| Продукт | Kie `resolution` | Kie `aspect_ratio` | Сколько createTask |
| --- | --- | --- | --- |
| Посты 12:12 / 21:21 | **1K**, один кадр | **1:1** | один still |
| Статьи Эскалибура + истории «Магия истории» | **2K**, холст 2×2 | **16:9** (не 1:1 / не square). После среза все 4 клетки 16:9 | **один** холст, потом срез. Не четыре 1K |
| Каруселька / carousel storyboard | **4K** | как в каноне карусели | как в каноне карусели |

Холст и нарезка: [`CANVAS_2K.md`](CANVAS_2K.md). Лицо Вики (истории = статьи): [`IMAGE_CANON.md`](IMAGE_CANON.md).
`input.aspect_ratio` и `input.resolution` — поля Kie `createTask`. Для холста статей/историй не опускать `aspect_ratio` (дефолт/auto → квадрат).
