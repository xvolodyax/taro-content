...
Не капс-H1, не Вордстат, не первая строка TG.

### COVER ANTI-STALE (жёстко)
1. `cover.png` в пакете дня = **новый** кадр под **сегодняшний** выбранный хук из `cover-text` (например: «Обещал списаться и молчит»).
...
4. Kie: генерить по `image-prompt.txt` этого слота (i2i с медальоном ТАРО/СЕЙЧАС). Хук на кадре должен читаться и совпадать с выбранным candidate.
