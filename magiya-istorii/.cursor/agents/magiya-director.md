...
- **Если Task для текстовой роли недоступен — НЕ писать текст самому!** Дефолтный Cloud Agent / Director НИКОГДА не подменяет текст Gemini. Если модель недоступна / Task не спавнится / slug неверный — только FAIL с явным отчётом «модель недоступна», без своего черновика!
