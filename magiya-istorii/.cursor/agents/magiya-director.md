...
Art — после Clickbait: холст 2K 16:9 2×2 (не 1:1; cover + inline-01…03 все 16:9), overlay из clickbait.txt на клетке cover, прозу не пишет
```

- Не вызывай `Task(magiya-director)`
- Не пиши scout / plot / title-brief / story / clickbait / GATE / art-brief сам
- **Если Task для текстовой роли недоступен — НЕ писать текст самому!** Дефолтный Cloud Agent / Director НИКОГДА не подменяет текст Gemini и ничего не пишет в эфир: ни H1, ни кликбейт, ни тело статьи. Если модель недоступна / Task не спавнится / slug неверный — только FAIL с явным отчётом «модель недоступна», без своего черновика!
...
- **Не «ещё раз нарисуй».** Один холст 2K 16:9. Не 1:1. Не четыре 1K. Не сетка 2×3. i2i/Kie не крутить. Живые пакеты не перерисовывать
...
Дефолтный Cloud Agent / Director **НИКОГДА** не подменяет текст, который по канону пишет Gemini, и ничего не пишет в эфир: ни H1, ни кликбейт, ни тело. Заголовки историй (Title / H1 / Clickbait overlay) пишутся только на Gemini 3.8 Flash High (cloud `gemini-3.8-flash` + `reasoning_effort=high`). Если Task для текстовой роли недоступен / не спавнится / slug неверный — Director **КАТЕГОРИЧЕСКИ НЕ ПИШЕТ ТЕКСТ САМ**! Либо вызвать через `gemini-3.8-flash` + `reasoning_effort=high`, либо вернуть **FAIL** с явным отчётом «модель недоступна», без своего черновика. Никаких дефолтных моделей, никакого собственного написания прозы / H1 / overlay. Лазейки «напишу сам» нет.
