...
- **Cloud Agent / launch:** model id `gemini-3.8-flash`, param `reasoning_effort=high` (заголовки историй Title / H1 пишутся строго на этой модели!).
- **Локальный Task (IDE):** slug `gemini-3.8-flash-high` зафиксирован только как alias для локальных вызовов.
- **Дефолтный агент в эфир не пишет ничего:** ни H1, ни кликбейт, ни тело статьи. Если модель недоступна / Task не спавнится / slug неверный — только **FAIL** («модель недоступна»), без своего черновика H1. Никаких дефолтных моделей, Grok, Composer, Claude.
