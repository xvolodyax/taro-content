...
3. Слово «ловушка». На 21:21 ещё «Сцена» / «Сцена:» в эфире, debrief, GATE, отчётах.
...
**21:21.** Не тизер статьи. Есть `tg.html`, `vk.html`, `yt.txt`. Нет `max.txt`, `ig-story.txt`, `ig.txt`. TG ≤ 1024.
Нет слова «Сцена» / «Сцена:» и нет ярлыков «кадр» / «хук» как заголовков. Первая строка — ситуация без имени.
В `debrief.md` четыре полных расклада: ответ = вопрос, карта, что показывает, развёрнутый совет. FAIL стикер «Карта. Закрой профиль.»
Подпись может быть плотнее debrief, но у каждого варианта должен быть совет карты, не одно действие. Карты случайные. YT без сырого URL. Cover + image-prompt есть. Пиксели не обязательны.
