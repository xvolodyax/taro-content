...
Писатели шагов: `publish: SKIP`. После `GATE` = PASS: один прогон скрипта **без** `--wait`.
Слот не наступил — `READY_TO_SEND` и **выход**. Не sleep, не poll, не Read-loop до слота.
Эфир в слот = Холл / короткий air wake.
Холл не публикует и не пишет тексты.
`preview: poll-only` — скрипт публикации **не** звать.
`evening: HOLD` — 21:21 не писать; опрос 15:15 после PASS готов, агент выходит.
...
Дефолтный Cloud Agent / Director НИКОГДА не подменяет текст воркера. Ни статьи magiya, ни посты 12:12/15:15/21:21, ни опросы, ни Алёна, ни рилсы-текст.
Если Task не спавнится / модель недоступна — только FAIL + явный отчёт «модель недоступна», без своего черновика. Лазейки «напишу сам» нет!
...
После PASS: `READY_TO_SEND` и выход. 07:00 МСК = Холл / air wake.
...
7. PASS обычного слота → сразу один раз:

```text
python3 scripts/posts_publish.py --package DIR
```

Без `--wait`. Нет `COMPOSIO_API_KEY` — скрипт пишет SKIP и выходит 0. Не падать.
Слот МСК не наступил — `READY_TO_SEND` и **EXIT**. Не ждать 12:12 / 15:15 / 21:21.
Слот прошёл — слать сразу и выйти.
