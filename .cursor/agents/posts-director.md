...
description: |
  ...
  21:21 = один Gemini 3.7, рубрика «Другая сторона экрана».
model: inherit
is_background: false
---
...
3. Один `Task(generalPurpose)` с этим промптом. Текстовые шаги: `model: gemini-3.7-flash-high`.
...
Потом **один** `Task(posts-copywriter)` / Cloud `Task(generalPurpose)` модель Gemini 3.7.
...
Предложения не гладить. Три позиции. Позиция 3 про неё. Пульс «Похоже? / Не то».
