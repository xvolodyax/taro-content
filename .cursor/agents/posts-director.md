...
**12:12.** Scout → Writer → Sol (пять площадок) → Gate → Cover-text.

**15:15.** Scout (та же сцена, что 12:12) → Writer: опрос **и сразу** `debrief.md` (4 случайные карты, затем 4 мини-расклада). Sol: только `tg.html` + `vk.html` опроса. Gate. Cover-text нет. Голоса не ждать.

**21:21.** Если есть `posts/YYYY-MM-DD-1515/debrief.md` — Scout/Writer-смысл **не** запускать. Sol: `tg.html` = `max.txt`, `vk.html` тем же текстом, ≤1024. Без IG/YT. Gate → Cover-text. Если debrief нет (опрос уже в эфире) — Writer один раз собирает debrief в пакете 21:21, не новый дневной смысл.
...
5. PASS и слот 12:12 или 21:21 → Task `posts-cover-text`
6. Стоп. Холлу: путь, `GATE`, на 21:21 ещё 4 карты и длина TG. Агент не публикует.
